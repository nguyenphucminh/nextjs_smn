...
export default function PartnerCustomer() {
  ...
  useEffect(() => {
    if (partners.length < 1) {
      fetch(`${process.env.NEXT_PUBLIC_API}${process.env.NEXT_PUBLIC_PREFIX_API ? `/${process.env.NEXT_PUBLIC_PREFIX_API}` : ''}/partners`)
          ...
          >
            {partners.map((partner, i) => (
              <div className={styles.slide_item} key={i}>
                <a href="#">
                  <img
                    src={`${process.env.NEXT_PUBLIC_API}${partner?.avartar?.url}`}
